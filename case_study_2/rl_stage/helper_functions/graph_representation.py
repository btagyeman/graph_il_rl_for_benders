...
#     n_nodes=self.n_variables + cons_total # number of nodes = number of variables + number of constraints
#     A_matrix=np.zeros((cons_total, self.n_variables))
...
#     var_node_features = np.array(self.past_values).reshape(-1,1)
...
#     y=np.array(self.optimal_values)
...
#         pass
#     # var_node_features = np.array(self.optimal_values[cons-1]).reshape(-1,1)  # Use the values of the complicating variables at the previous iter as node features 
#     var_node_features = np.array(past_values).reshape(-1,1)
#     # var_node_features = np.zeros(self.n_variables).reshape(-1,1) # TO DO: CHANGE THIS IF NEEDED
#     cons_node_features = b_vector.reshape(-1,1) 
#     x=np.concatenate((var_node_features,cons_node_features)) # nodes features 
#     edges = []
...
def create_graph_representation(cut_order,graph_features,past_values,n_variables):
    if len(graph_features['optimality_cuts'].values()) != 0 and len(graph_features['feasibility_cuts'].values()) != 0:
        cons_opt = list(graph_features['optimality_cuts'].values())
        cons_feas= list(graph_features['feasibility_cuts'].values())
        cons_total = len(cons_opt) + len(cons_feas)
    ...
    a=sp.coo_matrix((np.ones(edges.shape[1]),(edges[0], edges[1])), shape=(n_nodes, n_nodes))
    e=np.array(edge_features) 
    y=np.array(past_values) # this is not needed since we are not using labels in the graph representation, but we can keep it for consistency
    return Graph(x=x,a=a,e=e,y=y)