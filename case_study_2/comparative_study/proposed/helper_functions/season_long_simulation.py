...
def run_irrigation_scheduler(self, k):
    ...
    states_mz1,irrigAmnts_mz1,irrigDecs,ubd_final,lbd_final,optimal_values,graph_features,cut_order, eval_time_mp, bin_var_predicted, mp_iters, predicted_vars_agent=run_gbd_for_scheduler(x0_scaled_mz1,u_init_mz1,self.guessesX,self.guessesU,self.guessesC,
    ...
    self.mp_count.append(mp_iters)
    self.ol_mp_time.append(sum(eval_time_mp))
    self.cut_order = cut_order
    self.optimal_values = optimal_values
    self.prediced_binary_variables.extend(predicted_vars_agent)
    self.eval_time_mp.extend(eval_time_mp)
    for i in range(len(bin_var_predicted)):
        self.predictedIrrigations.append(bin_var_predicted[i])
        pass 
